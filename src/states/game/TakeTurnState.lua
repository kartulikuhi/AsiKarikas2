            ...
            Timer.after(1.5, function()
                ...
                :finish(function()
                    ...
                    if self.playerPokemon.currentExp > self.playerPokemon.expToLevel then
                        ...
                        hitp, atk, defe, sped = self.playerPokemon.HP, self.playerPokemon.attack, self.playerPokemon.defense,  self.playerPokemon.speed
                        a, b, c, d = self.playerPokemon:levelUp()
                        gStateStack:push(StatsLvlUpState(self:displayLvlUpStats(a, b, c, d)))
...
function TakeTurnState:displayLvlUpStats(hp, attack, defense, speed)
    self.stats = Menu {
        items = {
            {
                text = 'HP ' ..tostring(hitp) .. '+' .. tostring(hp) .. '=' ..tostring (hitp + hp)
            },
            {
                text = 'Atk ' ..tostring(atk) .. '+' .. tostring(attack) .. '=' ..tostring (atk + attack)
            },
            {
                text = 'Def ' ..tostring(defe) .. '+' .. tostring(defense) .. '=' ..tostring (defense + defe)
            },
            {
                text = 'Spd ' ..tostring(sped) .. '+' .. tostring(speed) .. '=' ..tostring (sped + speed)
            }
        },
        x = VIRTUAL_WIDTH - 128,
        y = 0,

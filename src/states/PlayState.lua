    ...
    if matches then
        ...
        for k, match in pairs(matches) do
            for k, tile in pairs(match) do
                local isRow = math.random(2)
                if tile.isShiny and isRow == 1 then
                    for x = 1, 8 do
                        self.score = self.score + (self.board.tiles[tile.gridY][x].variety - 1) * 25 + 50
                    end
                    self.board:removeRow(tile.gridY)
                    break
                elseif tile.isShiny and isRow == 2 then
                    for y = 1, 8 do
                        self.score = self.score + (self.board.tiles[y][tile.gridX].variety - 1) * 25 + 50
                    end
                    self.board:removeColumn(tile.gridX)
                    break
                else
                    self.score = self.score + (tile.variety - 1) * 25 + 50
                    self.timer = self.timer + 1
                end
            end
        end
